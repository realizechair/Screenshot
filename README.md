...
**v4新機能**: モード選択機能追加！シングルモード・マルチモード選択可能！
...
- **モード選択**: シングルモード（1枚固定）またはマルチモード（複数自由配置）
- **複数画像配置**: マルチモードで何枚でも画像を貼り付け可能
- **画像のリサイズ**: マルチモードで各画像を自由にサイズ変更可能
- **黒塗り機能**: 選択エリアを完全に黒く塗りつぶし
...
**ヒント**: 
- モード選択画面: 右上の「×」ボタンでデフォルトでマルチモードが開始
- 使い方ガイド: 右上の「×」ボタンでガイドを閉じる
...
3. **何度でも貼り付け可能！** 複数の画像を同じキャンバスに配置

#### 方法B: ドラッグ&ドロップ
- 画像ファイルをブラウザにドラッグ（繰り返し可能）

#### 方法C: ファイル選択
- 「📁 画像読み込み」ボタンをクリック（繰り返し可能）

### 2. 画像を配置・調整（NEW!）

**重要**: 画像は**原寸大（100%）**で配置されます

#### 画像の移動
1. 「↖️ 選択」ツールを選択（または `V` キー）
2. 画像をクリックして選択
3. ドラッグで好きな位置に移動
...
1. 画像を選択
2. 四隅の青いハンドルをドラッグ
3. 自由にサイズを変更可能（原寸大から拡大・縮小）

#### 画像の削除
- 画像を選択して `Delete` キーまたは「🗑️ 削除」ボタン
...
2. 配置したい場所をクリック
3. 自動で番号が増える(1, 2, 3...)
4. **番号をリセット**: 「🔄 番号リセット」ボタンをクリックで①に戻る

#### 黒塗り(NEW!)
1. 「⬛ 黒塗り」ボタンをクリック（または `B` キー）
2. マウスでドラッグして黒塗りするエリアを選択
3. マウスを離すと自動的に**完全な黒塗り**が適用される
4. 複数作成可能、移動・リサイズも可能
5. **用途**: 個人情報や機密情報の完全な隠蔽に最適
