...
**v3新機能**: 複数の画像を1つのキャンバスに配置・リサイズ可能！モザイク機能追加！

## 🎯 特徴

- **複数画像配置**: 1つのキャンバスに何枚でも画像を貼り付け可能（NEW!）
- **画像のリサイズ**: 各画像を自由にサイズ変更可能（NEW!）
- **モザイク機能**: 選択エリアにモザイク処理を適用可能（NEW!）
...
#### モザイク(NEW!)
1. 「🔳 モザイク」ボタンをクリック（または `M` キー）
2. **モザイクの粗さを調整**: 🔳スライダーで50px～300pxまで調整可能（デフォルト150px）
3. マウスでドラッグしてモザイクを適用するエリアを選択
4. マウスを離すと自動的にモザイク処理が適用される
5. 複数作成可能、移動・リサイズも可能
6. **粗さの目安**: 50px=細かい、150px=標準、300px=非常に粗い

#### 色と線の太さ変更(NEW!)
- 🎨 アイコンをクリックして色を選択
- スライダーで線の太さを調整(1-10px)
- 🔳 スライダーでモザイクの粗さを調整(50-300px)
